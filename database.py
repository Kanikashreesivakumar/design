...
def create_tables():
    ...
    conn.commit()
    conn.close()
    print("Repair log table checked/created successfully")
...
def edit_record():
    ...
    pending_count = get_repair_log_pending_count()
...
def show_dashboard():
    ...
    "Synchro": {"CCR Number plate": 1, "Foam/protector": 3}
        ...
        print(f"   Trolley types: {list(trolley_types)}")
        print(f"   Chart data keys: {list(trolley_charts.keys())}")
        print(f"   Repair count: {repair_count}")
        print(f"   Primary check count: {primary_check_count}")
        print(f"   Complete check count: {complete_check_count}")
...
def repair_log():
    ...
    conn = sqlite3.connect(DB_FILE)
    df = pd.read_sql('SELECT * FROM repair_log ORDER BY id', conn)
    conn.close()

    if df.empty:
        return render_template('repair_log.html', data=[], datetime=datetime)
    ...
    print(f" Displaying {len(df)} records from repair_log table")
...
def submit_action():
    ...
    try:
        ...
        if not record_id or not action_taken or not user_name:
            print(" Missing required fields")
            return render_template('repair_log.html', 
        ...
        old_count = get_repair_log_pending_count()
        ...
        c.execute('SELECT id, trolley_number, concern_description FROM repair_log WHERE id = ?', (record_id,))
        ...
        print(f"Found record: ID={existing_record[0]}, Trolley={existing_record[1]}")
        ...
        if rows_affected == 0:
            ...
            return render_template('repair_log.html', 
                                 error=f"Failed to update record {record_id}", 
                                 data=[], 
                                 datetime=datetime)
        ...
        # Verify update
        c.execute('SELECT id, trolley_number, concern_description, action_taken_by, action_time, action_status FROM repair_log WHERE id = ?', (record_id,))
        updated_record = c.fetchone()
        
        conn.close()
        
        new_count = get_repair_log_pending_count()
        
        print(f" Action submitted successfully!")
        print(f"   Pending count: {old_count} → {new_count}")
        ...
        traceback.print_exc()
        return render_template('repair_log.html', 
                             error=f"Failed to submit action: {e}", 
                             data=[], 
                             datetime=datetime)


def send_trolley_alerts():
    """Send alerts for trolleys that haven't received action within required timeframe"""
    try:
        json_tracker_path = "alerted_trolleys.json"
        alert_days = [3, 7]  # Send alerts after 3 and 7 days
        ...
        if df.empty:
            print(" No repair log data found for alerts")
            return
        ...
        for _, row in df.iterrows():
            trolley_name = str(row.get('trolley_number', 'Unknown'))
            comp_time = row.get('completion_time')
            action_time = row.get('action_time')

            # Parse completion time
            if pd.isna(comp_time) or comp_time == '':
                continue

            try:
                if isinstance(comp_time, str):
                    comp_time = pd.to_datetime(comp_time).date()
                elif isinstance(comp_time, datetime):
                    comp_time = comp_time.date()
                else:
                    continue
            except Exception:
                continue
            ...
            for day in alert_days:
                ...
                action_taken = pd.notna(action_time) and str(action_time).strip() != ''

                if not already_alerted and not action_taken and today >= due_date:
        ...
        for row, day in inactive_alerts:
            trolley = row.get('trolley_number', 'Unknown Trolley')
            concern = row.get('concern_description', 'No concern provided')
            comp_time = row.get('completion_time', 'Unknown Date')
            html_rows += f"""
                <tr>
                    <td>{trolley}</td>
                    <td>{concern}</td>
        ...
        receiver_email = "shanmugam.murugan@rnaipl.com"
        password = "cyqd dpvx jmjl xtbb"
        ...
        print(f"error in send_trolley_alerts: {e}")
        import traceback
        traceback.print_exc()
...
def update_repair_action(record_id, action_taken_by, action_taken, action_time):
    """Update repair log with action taken"""
    try:
        conn = sqlite3.connect(DB_FILE)
        c = conn.cursor()
        ...
        c.execute('''
            UPDATE repair_log 
            SET action_taken_by=?, action_status=?, action_time=?
            WHERE id=?
        ''', (action_taken_by, action_taken, action_time, record_id))
        ...
        if rows_affected > 0:
            print(f" Updated repair action for ID {record_id}")
            return True
        else:
        ...
        print(f" Failed to update repair action: {e}")
        return False
        ...
        for path in excel_paths:
            if os.path.exists(path):
                ...
                if not df.empty:
                    ...
                    result += "<tr>" + "".join([f"<th>{col}</th>" for col in df.columns]) + "</tr>"
                    for _, row in df.head(5).iterrows():
                        result += "<tr>" + "".join([f"<td>{row[col]}</td>" for col in df.columns]) + "</tr>"
                    result += "</table>"
                break
        else:
            result += f"<p> No Excel file found in any of these paths: {excel_paths}</p>"
...
@app.route('/debug-sqlite')
def debug_sqlite():
    """Debug route to check SQLite database contents"""
    try:
        conn = sqlite3.connect(DB_FILE)
        ...
        for col in columns:
            result += f"<tr><td>{col[1]}</td><td>{col[2]}</td></tr>"
        
        result += f"""
        </table>
        
        <h3>Data ({len(df)} rows):</h3>
        <table border='1'>
        """
        ...
        if not df.empty:
            ...
            result += "<tr><td colspan='100%'>No data found</td></tr>"
            
        result += "</table>"
        return result
        
    except Exception as e:
        return f"Error reading SQLite: {e}"





@app.route('/api/repair-log-count')
def api_repair_log_count():
    """API endpoint to get current repair log pending count"""
    count = get_repair_log_pending_count()
    return jsonify({'pending_count': count})

def get_repair_log_pending_count():
    """Get count of pending repair log items (where action is not completed)"""
    try:
        conn = sqlite3.connect(DB_FILE)
        c = conn.cursor()
        ...
        c.execute('''
            ...
            WHERE (action_taken_by IS NULL OR action_taken_by = '' OR action_status != 'Completed')
        ''')
        count = c.fetchone()[0]
        conn.close()
        
        print(f" Repair log pending count: {count}")
        return count
